...
bool ModelClass::Initialize(ID3D11Device * device, LPCSTR textureFile)
{
	bool result;
...
bool ModelClass::LoadTexture(ID3D11Device * device, LPCSTR file)
{
	bool result;
	...
	result = m_Texture->Initialize(device, file);
	if (!result)
		return false;
